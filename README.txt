...
2. 単純
	わずか 4000 行 (C 言語) ですので、セキュリティホールが生じる可能
	性を最小限にできます。
...
使用方法
	...
	ます。デフォルトは子プロセス無しです。-a を指定すると、アクセス
	ログを file へ出力します。-L を指定すると、エラーメッセージ等を 
	file へ出力します。-l を指定すると、エラーメッセージ等を syslog 
	へ出力します。

	-T を指定すると TCP セッションのタイムアウトの秒数を変更できます。
	デフォルトは 600 (10 分) です。-o と -g はそれぞれユーザ ID とグ
	ループ ID を指定します。-t を指定すると、dir へ chroot します。
